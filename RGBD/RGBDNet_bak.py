...
class Net_V2(nn.Module):
    ...
    def __init__(self,
        ...
        self.color_net = nn.Sequential(conv2d(channels_in - 1, 32, 1, stride=1, padding=0), nn.ReLU(),  #nn.MaxPool2d(2,2),
                                       conv2d(32, 64, 1, stride=1, padding=0), nn.ReLU(),  #nn.MaxPool2d(2, 2),
                                       conv2d(64, 128, 1, stride=1, padding=0), nn.ReLU(),  # nn.MaxPool2d(2, 2),
                                       #conv2d(128, 256, kernel_size, stride=1, padding=(kernel_size - 1) // 2)
                                       )
        self.head_net = nn.Sequential(nn.ReLU(),
                                      conv2d(256, 128, kernel_size, stride=1, padding=(kernel_size - 1) // 2),nn.ReLU(),
                                      conv2d(128, 64, kernel_size, stride=1, padding=(kernel_size - 1) // 2), nn.ReLU(),
                                      conv2d(64, 32, kernel_size, stride=1, padding=(kernel_size - 1) // 2), nn.ReLU(),
                                      conv2d(32, 1, 1, stride=1, padding=0), nn.ReLU())
...
class Net_V5(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1d = nn.Sequential(conv2d(in_channels=1, out_channels=64, kernel_size=7, stride=1, bias=False),nn.ReLU())
        self.conv2d = nn.Sequential(conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, bias=False),nn.ReLU())
    ...
    def forward(self, init_hr_c: Tensor, init_lr_d: Tensor) ->Union[Tensor,tuple]:
        ...
        a['conv2c'] = hr_c
        # 第1层
        lr_d = self.conv1d(self.down(init_lr_d))
        a['conv1d'] = lr_d
        # 第2层

...
def main():
    evaluator = Evaluator()
    model = Model_V3(weight_bit, output_bit, isint, clamp_std, noise_scale)
    path_to_pred, path_to_pred_fold = None, None
    path_to_conf, path_to_conf_fold = None, None

    if SAVE_PRED:
        path_to_pred = os.path.join("output", "test", "pred", "{}_{}".format("train" if USE_TRAINING_SET else "test", str(time.strftime('%Y%m%d_%H%M',time.localtime(time.time())))))

    if SAVE_CONF:
        path_to_conf = os.path.join("output", "test", "conf", "{}_{}".format("train" if USE_TRAINING_SET else "test", str(time.strftime('%Y%m%d_%H%M',time.localtime(time.time())))))
    ...
    for num_fold in range(3):
        ...
        path_to_pretrained = os.path.join("output/trained_models", MODEL_TYPE, "fold_{}".format(num_fold))
        ...
        with torch.no_grad():
            for i, (img, label, file_name) in enumerate(dataloader):
                ...
                if SAVE_PRED:
                    np.save(os.path.join(path_to_pred_fold, file_name[0]), pred.cpu())
                if SAVE_CONF:

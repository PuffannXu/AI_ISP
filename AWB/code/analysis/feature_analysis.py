...
def show_feature_map(
    ...
    row_num = np.ceil(np.sqrt(feature_map_num)).astype(int)  # 8
    plt.figure()
...
if __name__ == '__main__':
    image_dir = r"/home/project/xupf/Projects/FC4/dataset/preprocessed/gt_corrected/8D5U5524.png"
    # 定义提取第几层的feature map
    k = 0
    image_info = get_image_info(image_dir)

    model = Model()
    PATH_TO_PTH_CHECKPOINT = os.path.join("/home/project/xupf/Projects/AI_ISP/AWB/output/trained_models/Alex_V3_full")
    model.load(PATH_TO_PTH_CHECKPOINT)
    model_layer = list(model._network.children())
